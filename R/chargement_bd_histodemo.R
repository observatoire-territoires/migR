...
#' COMM_HISTODEMO_2016 <- chargement_bd_histodemo(chemin_FD = "./data/base-cc-serie-historique-2016/base-cc-serie-historique-2016.xls")
#' }
#'
#' @export
#'
  ...
  if(telechargement %in% TRUE){
    ...
    parametres_DL <-
      tibble::tribble(
        ...
        last_RP,"https://www.insee.fr/fr/statistiques/4171585","https://www.insee.fr/fr/statistiques/fichier/4171585/base-cc-serie-histo-2016.zip", "base-cc-serie-historique-2016"
      )
    ...
    output <- readxl::read_excel(paste0(dossier_dest_TL, "/",parametres_DL$nom_xls,"/",parametres_DL$nom_xls,".xls"), sheet = "COM_2016",skip = 5) %>%
      rename(#paste0("POP_",last_RP) = sym(paste0( substr(last_RP,3,4), "_RP")),
        ...
        !!sym(paste0("POP_",last_RP_antepe)) := !!sym(paste0("P", substr(last_RP_antepe,3,4), "_POP")),
        POP_1999 = D99_POP,
        POP_1990 = D90_POP,
        POP_1982 = D82_POP,
        POP_1975 = D75_POP,
    ...
    output <- readxl::read_excel(paste0(chemin_FD), sheet = "COM_2016",skip = 5) %>%
      rename(#paste0("POP_",last_RP) = sym(paste0( substr(last_RP,3,4), "_RP")),
        !!sym(paste0("POP_",last_RP)) := !!sym(paste0("P", substr(last_RP,3,4), "_POP")),
        !!sym(paste0("POP_",last_RP_ante)) := !!sym(paste0("P", substr(last_RP_ante,3,4), "_POP")),
        !!sym(paste0("POP_",last_RP_antepe)) := !!sym(paste0("P", substr(last_RP_antepe,3,4), "_POP")),
